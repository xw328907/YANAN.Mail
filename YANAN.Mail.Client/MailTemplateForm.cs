...
namespace YANAN.Mail.Client
{
    ...
    /// 窗体 - 邮箱模板设置
    /// </summary>
    public partial class MailTemplateForm : BaseChildForm
    {
        ...
        private void toolBtn_delete_Click(object sender, EventArgs e)
        {
            ...
            if (node == null)
            {
                MessageBox.Show("请选择要删除的模板", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information, MessageBoxDefaultButton.Button1);
                return;
            }
            if (MessageBox.Show("确定删除模板" + node.Text + "?", "删除提示", MessageBoxButtons.OKCancel, MessageBoxIcon.Asterisk, MessageBoxDefaultButton.Button1) != DialogResult.OK)
                return;
        ...
        public void LoadTreeMailSign()
        {
            ...
            if (combo_MailBoxId.SelectedItem != null)
                mailboxId = ((ListItem)combo_MailBoxId.SelectedItem).Value;
            var list = mailService.GetListSelfMailSign(CurrentUserInfo.GetLoginedUserInfo(), mailboxId, 2);
