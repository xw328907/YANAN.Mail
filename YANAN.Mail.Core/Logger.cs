...
namespace YANAN.Mail.Core
{
    ...
    public class Logger
    {
        ...
        /// 是否启用MailBee.NET日志
        /// </summary>
        public static bool MailBeeLogTrack = false;//AppConfigHelper.GetAppSetting("LogTrace") == "true";
        /// <summary>
        /// 启用MailBee.NET日志存储路径
        /// </summary>
        public static string MailBeeLogTrackFilePath = DefaultLogFolder;// AppConfigHelper.GetAppSetting("LogTraceFilePath");
        ...
        private static void Write(string text, string logFileNamePrefix, string logFolder)
        {
            ...
            if (string.IsNullOrEmpty(path))
            {
                ...
                if (string.IsNullOrEmpty(path))
                {
                    path = DefaultLogFolder + "/Log/Mail";
                }
            }
            string file;
            //如果目录不存在，创建这个目录
